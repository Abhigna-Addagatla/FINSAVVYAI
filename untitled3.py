...
Original file is located at
    https://colab.research.google.com/drive/1uazX3afunWZV0UR9tfKvEM75iZS_6kIv
"""
...
def generate_budget_interface(income, expenses, user_type):
    ...
    if not income or not expenses:
        return "⚠️ Please enter both income and expenses values."
    ...
    try:
        ...
        if income_val < 0 or expenses_val < 0:
            return "⚠️ Please enter positive values."
...
💸 Monthly Expenses: ${expenses_val:,.2f}
💵 Monthly Savings: ${income_val - expenses_val:,.2f}
📈 Savings Rate: {((income_val - expenses_val) / income_val * 100):.1f}%
...
def generate_spending_analysis(food, transport, entertainment, utilities, other, user_type):
    ...
    try:
        ...
        if total == 0:
            return "⚠️ Please enter at least one spending category."
...
with gr.Blocks(title="Personal Finance Chatbot", theme=gr.themes.Soft()) as demo:
    ...
    with gr.Tabs():
        ...
        with gr.Tab("📊 Budget Summary"):
            ...
            with gr.Row():
                with gr.Column():
                    ...
                    budget_expenses = gr.Number(label="💸 Monthly Expenses ($)", value=3500)
        ...
        with gr.Tab("💳 Spending Insights"):
            gr.Markdown("Get personalized insights on your spending patterns.")

            with gr.Row():
                with gr.Column():
                    spending_food = gr.Number(label="🍔 Food & Dining ($)", value=500)
                    spending_transport = gr.Number(label="🚗 Transportation ($)", value=300)
                    spending_entertainment = gr.Number(label="🎬 Entertainment ($)", value=200)
                    spending_utilities = gr.Number(label="💡 Utilities ($)", value=400)
                    spending_other = gr.Number(label="📦 Other ($)", value=600)
